...
class HawkT2VPipeline(DiffusionPipeline):
    r"""
    ...
    def __init__(
        ...
        self.vae_scale_factor = 8 #TODO : magic number 
    ...
    def __call__(
        ...
        negative_prompt_embeds = negative_prompt_embeds.to(torch.float32)
        ## added done
        if do_classifier_free_guidance:
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if do_classifier_free_guidance:
                    ...
                    # print(i, noise_pred_uncond.max(), noise_pred_uncond.min(), noise_pred_uncond.mean(), noise_pred_uncond.std(), noise_pred_text.max(), noise_pred_text.min(), noise_pred_text.mean(), noise_pred_text.std())
                    noise_pred = noise_pred_uncond + guidance_scale * (noise_pred_text - noise_pred_uncond)
                    print(i, noise_pred.max(), noise_pred.min(), noise_pred.mean(), noise_pred.std())
        ...
        print("final latents = ", latents.max(), latents.min(), latents.mean(), latents.std())
