...
def setup_distributed(backend="nccl", port=None):
    ...
    # timeout = dist.rpc.Timeout(seconds=3600) 
...
def check_fragmentation_and_clean(fragmentation_threshold=0.2, display=True):
    reserved_memory = torch.cuda.memory_reserved()
    allocated_memory = torch.cuda.memory_allocated()
    
    if reserved_memory > 0:  # 防止除以零错误
        ...
        if fragmentation > fragmentation_threshold:
            ...
            if display:
                print(f"After clearing cache - Fragmentation: {fragmentation:.2%}")   

    

def load_training_checkpoint(args, model_engine, PATH, ckpt_id=None):
    """Utility function for checkpointing model + optimizer dictionaries
       The main purpose for this is to be able to resume training from that instant again
    """
    _, checkpoint_state_dict = model_engine.load_checkpoint(PATH, tag=None if not ckpt_id else ckpt_id)
    epoch = checkpoint_state_dict['epoch'] if 'epoch' in checkpoint_state_dict else 0
    train_steps = checkpoint_state_dict['train_steps'] if 'train_steps' in checkpoint_state_dict else 0

    del checkpoint_state_dict
    return (epoch, train_steps)


def get_layer_gradient_norm(model, layer_name="final_layer.linear.weight"):
    ave_grad = None
    for n, p in model.named_parameters():
        if layer_name in n:
            grad = deepspeed.utils.safe_get_full_grad(p)
            ave_grad = torch.norm(grad.detach()) #.abs().mean().cpu().item()
    if ave_grad is None:
        print(f'layer {layer_name} not in the model', flush=True)
    return ave_grad

def get_gradient_norm(model_engine):
    unscaled_grad = None
    if  model_engine.optimizer.loss_scale != 0:
        unscaled_grad = model_engine.optimizer.scaled_global_norm()/ model_engine.optimizer.loss_scale
    if unscaled_grad is None:
        print(f'scaled grad norm is {model_engine.optimizer.scaled_global_norm()} but loss_scale is {model_engine.optimizer.loss_scale}', flush=True)
...
def to_device(batch, device, non_blocking=False):
    ...
    elif isinstance(batch, collections.defaultdict):
        return dict([
            (k, to_device(v, device, non_blocking))
            for k, v in batch.items()])
